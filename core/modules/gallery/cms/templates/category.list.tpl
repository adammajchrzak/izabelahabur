<div class="with-padding">
    ...
    <div class="float-right"><button onclick="document.location.href = '/cms#/{$router->getUrl('cms','gallery','cadd')}'" class="button button-add-text"><span class="button-icon green-gradient"><span class="icon-star"></span></span>dodaj</button></div>
    ...
    <table class="table">
        ...
        <tbody>
            ...
            {foreach item=tree name=tree from=$category_list}
                <tr>
                    <th><a href="/{$router->getUrl('cms','gallery','cedit',$tree.category_id)}" title="{$tree._name}">{$tree._name}</a></th>
                    <td>{$tree._description|truncate:45}</td>
                    <td nowrap="nowrap">
                        <button class="button compact green-gradient button-move-down" title="DOWN" value="{$tree.category_id}"><span class="icon-down-fat"></span></button>
                        <button class="button compact green-gradient button-move-up" title="UP" value="{$tree.category_id}"><span class="icon-up-fat"></span></button>
                    </td>
                    ...
                    <td nowrap="nowrap">
                        <button class="button compact icon-pencil blue-gradient button-edit" onclick="document.location.href = '/cms#/{$router->getUrl('cms','gallery','cedit',$tree.category_id)}'"></button>
        ...
        $(function () {
                        ...
                        'usuń': {
                            ...
                            click: function (modal) {
                                $.ajax({
                                    type: 'POST',
                                    async: false,
                                    url: '{/literal}/{$router->getUrl('cms','gallery','cdelete')}{literal}',
            ...
            $(".button-active").click(function () {
                $.ajax({
                    type: 'POST',
                    async: false,
                    url: '{/literal}/{$router->getUrl('cms','gallery','cactive')}{literal}',
            ...
            $(".button-move-up").click(function(){
                ...
                $.ajax({
                    type: 'POST',
                    async: false,
                    url: '{/literal}/{$router->getUrl($config->module_type,$config->module,'cmove')}{literal}',
